...
def split_dataset(input_dir, output_dir, test_size=0.15, val_size=0.15, random_state=42):
    ...
    for suku, images in all_images.items():
        ...
        # Salin gambar ke masing-masing split dengan mempertahankan struktur subjek
        for img_info in train_images:
            src_path = img_info['full_path']
            subjek = img_info['subjek']
            
            # Buat direktori subjek jika belum ada
            train_subjek_dir = os.path.join(train_suku_dir, subjek)
            os.makedirs(train_subjek_dir, exist_ok=True)
            
            # Salin gambar ke direktori subjek
            dest_path = os.path.join(train_subjek_dir, img_info['img_name'])
        ...
        for img_info in val_images:
            ...
            # Buat direktori subjek jika belum ada
            val_subjek_dir = os.path.join(val_suku_dir, subjek)
            os.makedirs(val_subjek_dir, exist_ok=True)
            
            # Salin gambar ke direktori subjek
            dest_path = os.path.join(val_subjek_dir, img_info['img_name'])
            shutil.copy2(src_path, dest_path)
        ...
        for img_info in test_images:
            ...
            # Buat direktori subjek jika belum ada
            test_subjek_dir = os.path.join(test_suku_dir, subjek)
            os.makedirs(test_subjek_dir, exist_ok=True)
            
            # Salin gambar ke direktori subjek
            dest_path = os.path.join(test_subjek_dir, img_info['img_name'])
            shutil.copy2(src_path, dest_path)
